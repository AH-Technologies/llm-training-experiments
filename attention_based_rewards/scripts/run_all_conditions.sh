...
#
# Usage:
#   bash attention_based_rewards/scripts/run_all_conditions.sh
#   DRY_RUN=1 bash attention_based_rewards/scripts/run_all_conditions.sh  # 2-step test
set -e
...
if [ ! -f "attention_based_rewards/data/gsm8k_train.parquet" ]; then
    ...
    srun --account=nn12068k --partition=accel --gpus=1 --cpus-per-task=8 --mem=32G --time=00:10:00 \
        bash -c "module load NRIS/GPU && module load Python/3.12.3-GCCcore-13.3.0 && \
                 ...
                 python attention_based_rewards/scripts/prepare_gsm8k_verl.py"
fi
...
for COND in uniform attention entropy combined; do
    ...
    JOB_ID=$(sbatch \
        ...
        attention_based_rewards/slurm/train_condition.slurm)
    echo "  Job ID: ${JOB_ID}"
done
