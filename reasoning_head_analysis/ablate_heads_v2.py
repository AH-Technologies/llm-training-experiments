...
Improvements over ablate_heads.py:
  - Full benchmark sweeps (MATH-500 500 problems, AIME'24 30, TriviaQA N)
...
MATH_COT_SUFFIX = " Let's think step by step and output the final answer within \\boxed{}."
...
def load_triviaqa(n=None):
    ...
    for i, row in enumerate(ds):
        ...
        probs.append({"idx": i, "question": row["question"],
                      "answer": json.dumps(aliases),
                      "category": "trivia", "difficulty": ""})
    return probs


BENCHMARKS = {"math500": load_math500, "aime24": load_aime24, "triviaqa": load_triviaqa}
...
def build_prompt(tokenizer, question, benchmark):
    if benchmark == "triviaqa":
        # Closed-book QA — keep responses short; CoT not useful here.
        msgs = [{"role": "user",
                 "content": f"{question}\nAnswer with the shortest possible factual phrase."}]
    else:
        msgs = [{"role": "user", "content": question + MATH_COT_SUFFIX}]
...
def grade(response, answer, benchmark):
    ...
    if benchmark == "triviaqa":
        try:
            aliases = json.loads(answer)
        except Exception:
            aliases = [answer]
        # Crude extraction: take last non-empty line, strip formatting.
        lines = [l.strip() for l in response.strip().split("\n") if l.strip()]
        if not lines:
            return 0
        pred = lines[-1].lower().strip(" .\"'")
        for a in aliases:
            if a and a.lower() in pred:
                return 1
        return 0
    try:
...
def plot_cross_benchmark(root_dir, bench_list):
    ...
    colors = {"math500": "steelblue", "aime24": "darkorange", "triviaqa": "seagreen"}
...
def main():
    ...
    parser.add_argument("--benchmarks", default=None,
                        help="Pipe- or comma-separated list, e.g. "
                             "'math500|aime24|triviaqa'. Each benchmark writes to "
    ...
    parser.add_argument("--triviaqa_n", type=int, default=200)
