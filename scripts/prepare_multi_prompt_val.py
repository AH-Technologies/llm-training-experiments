...
1. "math_no_cot"    — raw question only, no CoT instruction
2. "math_train_cot" — training-style suffix appended to user message:
                       "Let's think step by step and output the final answer within \\boxed{}."
3. "math_qwen_cot"  — Qwen/Alibaba CoT template as a system message (matching their eval):
                       system: "Please reason step by step, and put your final answer within \\boxed{}."
                       This matches the PROMPT_TYPE="qwen25-math-cot" from the One-Shot-RLVR eval.
...
def strip_existing_cot_suffix(content: str) -> str:
    """Remove known CoT suffixes if already present in the prompt."""
    suffixes_to_strip = [
...
def make_prompt_no_cot(raw_content: str) -> list[dict]:
    """Raw question only, no CoT instruction."""
    return [{"role": "user", "content": raw_content}]


def make_prompt_train_cot(raw_content: str) -> list[dict]:
    """Training-style: CoT suffix appended to user message."""
    return [{"role": "user", "content": raw_content + TRAIN_COT_SUFFIX}]


def make_prompt_qwen_cot(raw_content: str) -> list[dict]:
...
def create_multi_prompt_val(input_path: Path, output_path: Path):
    ...
    for _, row in df.iterrows():
        ...
        if isinstance(prompt_msgs, list):
            # Get user message content (skip any existing system message)
            user_msgs = [m for m in prompt_msgs if m.get("role") == "user"]
            base_content = user_msgs[-1]["content"] if user_msgs else prompt_msgs[-1]["content"]
        else:
            base_content = str(prompt_msgs)

        # Strip any existing CoT suffix to get the raw question
        raw_content = strip_existing_cot_suffix(base_content)
        ...
        for data_source, build_prompt in PROMPT_BUILDERS:
            ...
            new_row["prompt"] = build_prompt(raw_content)
